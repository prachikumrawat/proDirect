...
class ProdirectSpider(CrawlSpider):
    ...
    def parse_item(self, response):
        ...
        brand_name = response.xpath('//*[@id = "content"]//div/@data-brand').extract_first()
        restriction_boolean = self.restriction.parse_brands(brand_name)

        if restriction_boolean == False:
            item_dict = {}

            # product_ref_code =  response.xpath('//*[@class = "product-info"]//ul//li/text()').extract_first()
            product_asin = response.xpath('//*[@id = "content"]//div/@data-quickref').extract_first()
            item_dict['Product Code/SKU'] = product_asin
            item_dict['Product Name'] = response.xpath('//*[@class="right-column"]/h1/text()').extract_first()
            ...
            item_dict['Cost Price'] = cost_price
            selling_price = cost_price + 500.00 + (cost_price * .15) # Shipping_Cost = 500.00 , Profit = 15/100
            item_dict['Selling Price'] = float(selling_price)
            item_dict['Product URL'] = response.url
            if response.xpath("//*[@id='content']/div/div[4]/div[1]/div/ul/li[1]"):
                ...
                item_dict['Description'] = ''.join(description).strip()

            images_xpath = response.xpath("//*[@class='product-slider-holder']//li/img/@src").extract()
            item_dict['Product Image1'] = 'http://www.prodirectrunning.com/'+images_xpath[0]
            item_dict['Product Image2'] = 'http://www.prodirectrunning.com/'+images_xpath[1]
